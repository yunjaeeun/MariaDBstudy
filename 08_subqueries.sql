...
-- employee 에서 가장 높은 급여를 받는 사원 조회하기(MAX(컬럼명))
SELECT
		 emp_name
	  , MAX(salary)
  FROM employee;
...
-- 평균 급여가 가장 높은 부서 조회하기(AVG(컬럼명), MAX(컬럼명))
-- all을 활용한 방법
-- 서브쿼리 중에 다중행 서브쿼리인 경우에는 비교 연산자가 일반 비교연산자와 달라진다.
-- > all, < all, > any, < any, in
-- 1. > all : 모든 서브쿼리 결과보다 크다 (서브쿼리의 최대값 보단 크다)
-- 2. < all : 모든 서브쿼리 결과보다 작다 (서브쿼리의 최소값 보단 작다)
-- 3. > any : 서브쿼리 결과보다 최소 하나보다는 크다(서브쿼리의 최소값 보다 크다)
